...
High_Accuracy::High_Accuracy(int integer)
	:m_length(0)
{
	for (int i = 999, front = 1; i > 0; i--)
	{
		m_bit[i] = int(integer / (int)pow(10, i - 1));
		m_bit[i] %= 10;
...
void High_Accuracy::Input_Num()
{
	...
	for (int i = 1; i <= m_length; i++)
	{
		m_bit[i] = (int)s[m_length - i] - (int)'0';
	}
}
...
High_Accuracy High_Accuracy::Division(High_Accuracy& divisor)
{
	...
	remainder.Copy(*this, this->m_length - divisor.m_length + 1);
	for (int i = this->m_length - divisor.m_length + 1; i > 0; i--)
	{
		while (remainder.compare(divisor) == 1)
		{
			a.m_bit[i]++;
			remainder = remainder.Subtraction(divisor);
		}
		remainder.Move(1);
		remainder.m_bit[1] = this->m_bit[i - 1];

	}


	a.m_length = this->m_length - divisor.m_length + 1;
	if (!a.m_bit[m_length]) m_length--;

	return a;
}

void High_Accuracy::Move(int len)
{
	m_length += len;
